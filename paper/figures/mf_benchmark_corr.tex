% Généré par build_mf_benchmark_data.py — ne pas éditer à la main.
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lrl}
\toprule
Benchmark & Corr. & Common window \\
\midrule
SG CTA Index & 0.57 & 2000-01--2025-12 (312 mo.) \\
SG Trend Index & 0.55 & 2000-01--2025-12 (312 mo.) \\
Barclay BTOP50 Index & 0.59 & 2000-01--2025-12 (312 mo.) \\
KMLMSIM (Testfolio simulation) & 0.53 & 2000-01--2025-12 (312 mo.) \\
DBMF ETF (live) & 0.61 & 2019-06--2025-12 (79 mo.) \\
KMLM ETF (live) & 0.63 & 2021-01--2025-12 (60 mo.) \\
\midrule
\multicolumn{3}{p{0.82\textwidth}}{\footnotesize \textit{Memo, 2000--2025:} the three commercial indexes correlate 0.96--0.97 with each other, whereas KMLMSIM correlates 0.60 with SG CTA. The proxy correlates 0.55--0.59 with the three commercial indexes.} \\
\bottomrule
\end{tabular}
